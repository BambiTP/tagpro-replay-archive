...
<h2>What this is</h2>
<p>A record of every ranked TagPro match I can get at: <strong>{f(ids)} matches</strong> since
{span[0]}, each with its id, start time, map and duration, and a tagpro.eu record for {f(record)}
of them. For <strong>{f(rep)}</strong> I also have the recording itself.</p>
<p>I wanted this data and nobody had it. Ranked games happen, get listed for a couple of days, and
then you can't query them any more. So I started saving them.</p>

<h2>Why there are so few recordings</h2>
<p>I can't get the old ones. Scraping replays older than two days isn't allowed, and the site
won't show you a replay older than two days without an account. I asked for a bulk download of
everything already past that point and was turned down.</p>
<p>So I take them as they happen. The ids are easy: the listing hands over whole date ranges at
once, which is why that side is complete. The recordings are one file per match, fetched while the
match is still inside the window, which is why that side is not. Everything older than two days
when I started was already out of reach.</p>
<p><strong>Every week from here on gets collected in full. The old matches aren't coming back on
their own.</strong></p>

<h2>Send me your replays</h2>
<p>If you have replay dumps sitting around &mdash; exports, scrapes, a folder from two years ago,
anything &mdash; send them to <strong>{DISCORD}</strong> on Discord. Any format, any size, any
dates, however messy.</p>
<p>I match on uuid, so duplicates cost nothing and you don't need to check what I already have or
filter anything out first. A dump where 90% is already here is still 10% I didn't have. There is a
<a href="download.html">list of what is missing</a> if you want to aim at the gaps, but don't let
that stop you sending the whole thing.</p>

<h2>Where the numbers come from</h2>
<ul>
<li><strong>The ranked replay listing</strong> decides what exists. Every id on this site came from
it, so the totals are counts, not estimates.</li>
...
returns nothing for them, even inside ranges it says are complete. There's no recording to get.</li>
</ul>
...
<h2>How it runs</h2>
<p>A pipeline on a machine at my house follows the listing, pulls each replay while it is still
there, links it to its tagpro.eu record, and rebuilds records for matches the mirror missed. This
site is regenerated from that database on a schedule. The {held_bytes / 1e9:.1f} GB of recordings
stay on that machine and are served from it &mdash; see <a href="download.html">Download</a>.</p>
</section>'''
